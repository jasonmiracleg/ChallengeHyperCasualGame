...
class BackgroundManager {
    ...
    func addStaticTopOverlay(named imageName: String, scale: CGFloat = 1.0) {
        guard let scene = scene else { return }
        ...
        let scaledHeight = overlay.size.height * overlay.yScale
        overlay.position = CGPoint(x: 0, y: scene.size.height / 2 - scaledHeight / 2)
        overlay.name = "background_overlay"
